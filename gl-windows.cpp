...
namespace gl {
	...
	bool compileGLShader(std::string vertSrc, std::string fragSrc, GLuint* outVertexShader, GLuint* outFragmentShader) {
	...
	LRESULT CALLBACK onWindowMessage(
		HWND windowHandle,
	...
	void createWindowsWindow(const char* title, int w, int h, HWND* outWindowHandle) {
	...
	void createGLContext(HDC deviceContext, HGLRC* outGLContext) {
	...
	void createGLQuadVAO(GLuint* outVAO) {
		float quad[] = {
			// (x, y)
			-1.0f,  1.0f, // top left
	...
	void createContext(HDC* outDeviceContext, HGLRC* outRenderContext) {
		...
		*outDeviceContext = gdiDeviceContext;
		*outRenderContext = glRenderContext;
	...
	void createBuffer(size_t size, GLuint* outBuffer) {
